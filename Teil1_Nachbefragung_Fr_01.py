...
def likert_counts(series, order):
    """Zählt absolute Häufigkeiten einer Likert-Serie, inklusive -1."""
...
def plot_likert_bar(counts, title, ylabel="Anzahl", color="tab:blue", annotate=True, save=None):
    """Einfacher Balkenplot für eine Likert-Verteilung."""
...
plot_likert_bar(
    counts_all,
    title="Selbsteinschätzung (Nach_Fr_01) – Overall",
    color="skyblue",
...
plot_group_comparison(
    counts_by_gender,
    title="Selbsteinschätzung (Nach_Fr_01) – Geschlecht",
...
plot_group_comparison(
    counts_by_age,
    title=f"Selbsteinschätzung (Nach_Fr_01) – Alter (<{AGE_CUT} vs. ≥{AGE_CUT})",
